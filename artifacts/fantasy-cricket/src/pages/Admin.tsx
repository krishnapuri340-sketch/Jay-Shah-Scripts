                      ...
                      <div>
                        <div style={{ position: "relative" }}>
                          ...
                          <input
                            type="text" inputMode="numeric" pattern="\d{4}" maxLength={4}
                            value={val}
                            onChange={e => { setPinEditVal(e.target.value.replace(/\D/g, "").slice(0, 4)); setPinConfirmError(false); }}
                            autoFocus
                            style={{ position: "absolute", inset: 0, opacity: 0, cursor: "text", width: "100%", height: "100%" }}
                          />
                        </div>
                        {/* C1 fix: show error if server rejected the save */}
                        {pinConfirmError && (
                          <div style={{ textAlign: "center", color: "#f87171", fontSize: "0.62rem", marginTop: 10, letterSpacing: "0.3px" }}>
  ...
  {isCommissioner && (
    <>
      ...
      <div className="admin-btn-row">
        ...
        {/* C1 fix: replaced confirm() with inline confirmation UI */}
        {resetConfirming ? (
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            <span style={{ fontSize: "0.62rem", color: "#f87171" }}>Reset points?</span>
            <button
            ...
            <button
              ...
              style={{ fontSize: "0.62rem", padding: "5px 10px", borderRadius: 7, border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: "var(--text-3)", cursor: "pointer", fontFamily: "inherit" }}>
              Cancel
            </button>
          </div>
        ) : (
          <button className="btn-danger" onClick={() => setResetConfirming(true)}>
            Reset Cache
          </button>
        )}
      </div>
